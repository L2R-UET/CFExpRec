...
class SimGCL(GraphRecBaseModel):
    def __init__(self, data_handler, device, args, config, **kwargs):
        super(SimGCL, self).__init__(data_handler, device, args, config, **kwargs)
        self.cl_rate = getattr(config, "cl_rate", 0.2)
        self.eps = getattr(config, "noise_eps", 0.1)
        self.n_layers = getattr(config, "n_layers", 3)
        self.latent_dim = getattr(config, "latent_dim", 64)
        self.decay = getattr(config, 'decay', 1e-4)
        self.tau = getattr(config, "tau", 0.2)
        self.init_embedding()
    ...
    def propagate(self, norm_adj_mat, training=False):

        ego_embeddings = self.E0.weight
        all_layer_embeddings = []
        for _ in range(self.n_layers):
            if norm_adj_mat.is_sparse:
                E_layer = torch.sparse.mm(norm_adj_mat.to(self.device), ego_embeddings)
            else:
                E_layer = torch.mm(norm_adj_mat.to(self.device), ego_embeddings)
            if training:
            ...
            all_layer_embeddings.append(E_layer)

        all_layer_embeddings = torch.stack(all_layer_embeddings)
        mean_layer_embedding = torch.mean(all_layer_embeddings, axis=0)
    ...
    def forward(self, users, pos_items, neg_items):
        final_user_embed, final_item_embed = self.propagate(self.ori_norm_adj_mat, training=True)
